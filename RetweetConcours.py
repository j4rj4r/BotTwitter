        ...
        for tweet in tweepy.Cursor(api.search,q=mot + " since:" + time.strftime('%Y-%m-%d',time.localtime()),lang="fr",tweet_mode="extended").items(NombreDeRetweet): #On cherche avec #concours parmis les plus populaires en france
            try:
                if tweet.retweet_count > 5 :
                    ...
                    if re.search("\b(\w*INVIT(E|É)\w*)\b", tweet.full_text.upper()) : #On vérifie avec une expression régulière si il faut inviter des amies.
                        commentaire(api,tweet,tabname)
                    elif re.search("\b(\w*TAG\w*)\b", tweet.full_text.upper()) : #On vérifie si il faut inviter des amies.
                        commentaire(api,tweet,tabname)
                    elif re.search("\b(\w*MENTIONN(E|É)\w*)\b", tweet.full_text.upper()) : #On vérifie si il faut inviter des amies.
                        commentaire(api,tweet,tabname)
